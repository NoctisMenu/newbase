fn main() {
    ...
    let root_dir = app_dir
        .parent()
        .expect("Expected app crate to live under workspace root");

    let schema_path = app_dir.join("config_schema.toml");
    let keys_path = root_dir.join("src/app/config_system/keys.rs");
    let macros_path = root_dir.join("src/app/config_system/macros.rs");
